...
from app.models.cliente import obtener_cliente_por_id, eliminar_cliente
...
def eliminar(id):
    ...
    # 2. BUSCAMOS AL CLIENTE ANTES DE BORRARLO
    # (Usamos la función que ya tenemos en el modelo)
    cliente = obtener_cliente_por_id(id)

    if cliente:
        # 3. Guardamos el nombre en una variable
        nombre_cliente = cliente['nombre']

        # 4. Ahora sí, procedemos a eliminarlo de la base de datos
        eliminar_cliente(id)

        # 5. Mostramos el mensaje con el nombre real
        flash(f"El cliente '{nombre_cliente}' ha sido eliminado correctamente ✅")
    else:
        # En caso de que alguien intente borrar un ID que no existe
        flash("Error: No se encontró el cliente a eliminar.")
